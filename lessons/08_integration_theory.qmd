...
## R: Canonical Correlation Analysis (CCA)
...
## Python: single-cell Variational Inference (scVI)

[scVI](https://docs.scvi-tools.org/en/stable/) is a variational autoencoder that was published in [2018 by Lopez et. al](https://www.nature.com/articles/s41592-018-0229-2) to reduce technical noise and integrate disparate batches together. The method utilizes a variety of artificial intelligence methods to accomplish this task. To better understand the steps being used, we will first define what an autoencoders is, build upon the concept to define what variational autoencoders are, before lastly going through the scVI algorithm.
...
::: {#fig-vae_schematic .figure}
![](../img/autoencoder-architecture.png){width=90%}

Schematic of the architecture for an autoencoder, showcasing the encoder and decoder neural networks with the central latent space.<br>
_Image source: [Lil’Log](https://lilianweng.github.io/posts/2018-08-12-vae/)_
:::

Ultimately, the goal of this method is to calculate the latent space. As we know what the input looks like, we also know what the expected output is as well. The encoder and decoder go back and forth and update the values in the bottleneck until the output reconstructs the original input. This back and forth is part of the training step, which then allows you to apply the architecture to larger datasets.

### Variational autoencoders

Variational autoencoders build upon these concepts by computing the mean ($\mu$) and standard deviation ($\sigma^{2}$) for each input. Typically, this distribution is Gaussian.

::: {#fig-vae_architecture .figure}
![](../img/vae_architecture.webp){width=80%}

Schematic of the architecture for a variational autoencoder.<br>
_Image source: [Roger Yong](https://medium.com/geekculture/variational-autoencoder-vae-9b8ce5475f68)_
:::

So at this point, we have multiple distributions to represent the original input. However, what we ultimately want is that low-dimensional representation ($z$) of our input. So the guassian distribution is randomly sampled and used as the latent space before being passed to the decoder.

### scVI algorithm

As mentioned earlier, scVI is a variational autoencoder. The inputs supplied are the counts matrix as well as any batch information (sample, phase, mitochondrial ratio).
...
The broad steps of the scVI algorithm are as following:

1. We assume there is a low-dimensional latent space $z$ that can represent our dataset (similar to PCA).
2. The raw counts matrix and batch information are fed into the encoder, with the genes modelled as a zero-inflated negative binomial distribution.
3. This distribution is fed into the decoder with the batch labels to try and reconstruct the original counts matrix.
4. Multiple back and forths between the encoder and decoder to update the distributions such that the counts are reconstructed accurate.

  - **Encoder:** counts + batch -> $z$
  - **Decoder**: $z$ + batch → reconstructed counts 

5. Once the counts are reconstructed, the posterior mean of the $z$ distribution for each cell is used as the latent space values (which can then be used to downstream analyses).
