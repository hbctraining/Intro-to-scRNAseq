...
Now that we have our high quality cells integrated, we want to know the different cell types present within our population of cells. To this end, our next steps will be to identify similarity between our cells using networks. With this representation, we can run clutering and UMAP algorithms to represent transcriptionally unique populations of cells in our dataset.
...
The `cells` argument for `DimHeatmap()` specifies the number of cells with the most negative or postive PCA scores to use for the plotting. 
...
TODO: there is no anaalgous to plot this simply with scanpy. Should we just print the plot and move on?
...
kNN are calculate in one step with the `sc.pp.neighbors()` function. Importantly, we must specify the `use_rep` argument to be the integrated `X_scvi` latent space that was computed in the previous lesson.
...
We specified `key_added` as "scvi" so that we do not overwrite any previous neighborhoods that may have calculated. This can more clearly be seen when we look at the `obsp` slot of our adata object.
...
#| fig-cap: UMAP plot, representing each cell as a colored point corresponding each sample.
# Plot the UMAP
...
Effectively, we are summarizing the information accross multiple PCs into 2D space for this representation.
...
By default, the `FindClusters()` function using Louvain clustering. So, we will set `algorithm = 4` in order to make use of the Leiden implementation.
...
2. check the object clusters with different resolution (0.4, 0.6, 0.8, 1.0, 1.4). For each resolution plot the corresponding UMAP and report how many clusters you observe.
:::
