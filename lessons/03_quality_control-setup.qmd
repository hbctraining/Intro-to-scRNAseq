...
description: |
  In this lesson, participants learn how to load single‑cell RNA‑seq count data. The lesson introduces the structure of typical 10X Genomics output files, demonstrates how to import these files efficiently and shows how to generate objects for one or multiple samples. Participants also practice using loops to streamline data loading and explore the metadata. By the end, participants will be able to prepare their data for downstream quality control and analysis.
author:
  - Mary Piper
  - Meeta Mistry
  - Radhika Khetani
...
- Setup the environment for a single-cell analysis.
- Establish good data managements and metadata practices.
...
3. Unzip the `single_cell_rnaseq` folder and open it.

::: {#fig-project-structure .figure}
![](../img/proj_screenshot.png){width=600}


TODO: make this without the Rproj file
...
TODO: flush out instructions on opening Anaconda and creating a Jupyter Notebook
...
Save the file as `quality_control.ipynb`. Your working directory should look something like this:

TODO: picture of jupyter notebook setup
...
TODO: introduction to the AnnData object
...
- Sample or identity:
    ...
    - In scanpy/AnnData: can be stored in `obs["sample"]` or another column you define.
- Number of UMIs per cell:
    - In Seurat: `nCount_RNA`
    - In scanpy/AnnData: `total_counts`
- Number of genes detected per cell:
    - In Seurat: `nFeature_RNA`
    - In scanpy/AnnData: `n_genes`

## Reading in multiple samples with a `for loop`

In practice, you will likely have several samples that you will need to read in data for, and that can get tedious and error-prone if you do it one at a time. So, to make the data import into more efficient we can use a `for` loop, which will iterate over a series of commands for each of the inputs given and create scanpy/seurat objects for each of our samples. 
...
::: {.callout-note collapse=true}
# Breaking down the `for` loop
TODO: breakdown of loop for scanpy
:::

:::

Now that we have created two objects for each of the samples, let's take a quick look at the list. We should see that there are two Seurat/scanpy objects in our list that correspond to each sample.
...
TODO: more description of concat function
