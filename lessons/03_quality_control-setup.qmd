---
title: "Loading Single-cell RNA-seq Data"
description: |
...
# Seurat v5.5.1 broke something in Read10X() for me
# data.table::fread() does not work as intended anymore
# Putting these options in allows the page to render
...
The count data for this dataset is also freely available from 10X Genomics and in several popular tutorials, including the [Seurat PBMC tutorial](https://satijalab.org/seurat/v3.0/immune_alignment.html). 
:::
...
:::{.callout-tip}
# [**Exercise 1**](03_quality_control-setup-Answer_key.qmd#exercise-1)
...
::: callout-important
# Download data
If you haven't done this already, the project can be accessed using [this link](https://www.dropbox.com/scl/fi/pnnlp0pyaqbrqbfyfoidu/scrna_python_r_update.zip?rlkey=gz03gsqvg0l74fsxnwalw86mm&dl=1).
:::
...
![](../img/py_anaconda_nav.png){width=600}

Anaconda Navigator, with conda environment and launch button for Jupyter Lab highlighted.
:::
...
::: {#fig-anaconda_file_explorer .figure}
![](../img/anaconda_file_explorer.png){width=400}

File explorer with Jupyter Lab, navigating to where the downloaded `scrna_python_r_update` folder is stored. 
:::
...
::: {#fig-anaconda_notebook .figure}
![](../img/anaconda_new_notebook.png){width=400}

Create a new "Notebook" from the "File" tab of Jupyter Lab.
:::
...
::: {#fig-anaconda_kernel .figure}
![](../img/anaconda_kernel.png){width=400}

Select the `intro_scRNA` kernel for the Jupyter Notebook.
:::

Rename the notebook to be `quality_control.ipynb`.

::: {#fig-anaconda_rename .figure}
![](../img/anaconda_rename.png){width=800}

Rename the new `Untitled.ipynb` file to `quality_control.ipynb`.
:::

In the first cell, we are going to add some comments to indicate what this file is going to contain:

```{python}
#| label: script_metadata_py
#| eval: false
# Load 10x single-cell data
# Introduction to single-cell RNA-seq workshop
# Author: Harvard Chan Bioinformatics Core
...
::: {#fig-anaconda_notebook_setup .figure}
![](../img/anaconda_notebook_setup.png){width=800}

Final set-up of Jupyter Notebook for the scRNA-seq workshop.
:::
...
::: {.callout-note collapse="true"}
# Why don't we use the `filtered_feature_bc_matrix` folder?
...
```{r}
#| label: read_in_single_dataset_r
...
```{r}
#| label: tbl-ctrl_metadata_r
#| tbl-cap: First several rows of a the `ctrl@meta.data`
#| echo: false
...
[Scanpy](https://scanpy.scverse.org/en/stable/) is the Python package that we will be using. It is built upon an `AnnData` object, which is a data structure that can efficiently store our single-cell information. It has has slots that are meant to contain very specific pieces of information, shown here:

::: {#fig-anndata .figure}
![](../img/anndata.svg){width=600}

Schematic of the AnnData object with main components `.X` counts matrix, `.obs` metadata dataframe, and `.var` gene dataframe.<br>
_Image source:_ [Scanpy Usage Principles](https://scanpy.scverse.org/en/stable/usage-principles.html)
:::
...
```{python}
#| label: read_in_single_dataset_py
...
```{python}
#| label: tbl-show_metadata_py
#| tbl-cap: First several rows of `ctrl.obs`
#| echo: false
#| results: asis
# Calculate QC metrics for each cell
sc.pp.calculate_qc_metrics(ctrl,
                           ...
                           log1p = False,
                           inplace = True)
print(ctrl.obs.head().to_markdown())
```
...
|---------------|-------------|---------------------------------------------------------------------------------------------------------------------------------|
| `join`        | `"outer"`   | Keep the union of all genes across Scanpy objects; genes missing from a sample get filled with 0/`NaN`s.                        |
...
```{python}
#| label: tbl-show_meta_merged_adata_head_py
...
```{python}
#| label: tbl-show_meta_merged_adata_tail_py
...
[Next Lesson >>](04_cellranger_QC.qmd)
