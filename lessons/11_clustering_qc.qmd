...
```{python}
#| label: tbl-prop_df
#| tbl-cap: First several rows of `prop_df` which contails proportions of cells belonging to each sample and cluster.
#| echo: false
#| results: asis
...
#| fig-cap: UMAP splity cell cycle phase with cluster identity side-by-side.
#| fig.width: 3
#| fig.height: 3
# Create plot with the correct dimensions
fig, axes = plt.subplots(1, 3, figsize=(15, 5))
...
When we look at the boxplot representation, we can also see that cluster 16 also has higher than average `n_genes`. This is on the benefits of using different visuals to understand your data. 
...
We can utilize the UMAP plotting fuctions of these known genes to explore the expression of known gene markers in each cluster. Let us go through and start determining the identites of the clusters.
...
At first glance it may be challening to identify where this subtype of dendritic cells are, but we can once again use our violin plot to see if we can find a suitable cluster.

```{r}
#| label: fig-plasmacytoid_dendritic_cell_plot_vln
#| fig-cap: Violin plot of top Plasmacytoid dendritic cell markers.
#| fig.height: 6
VlnPlot(seurat_integrated,
        c("MARCO", "ITGAM", "ADGRE1"),
        ncol = 1)
