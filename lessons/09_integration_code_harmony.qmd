...
We can see the basic information about the model by printing out the `model` varaible created:
...
scVI uses the concept of variational autoencoders to build neural networks (encoder and decoder) that is capable to reconstructing the original counts information. To accomplish this goal, several back and forth interations between the two networks is necessary in order to ensure the accuracy of the reconstrucion.

::: callout-note
# Epochs
The number of epochs specifies how many iterations of the model training will be done. The more epochs will improve the results of the training. Here we have set it to 100 to _reduce the amount of time_ this step takes to run while retaining the accuracy of the results. 
