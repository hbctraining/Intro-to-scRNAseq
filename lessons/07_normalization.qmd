...
# Load dataset
filtered_adata = sc.read_h5ad("data/filtered_adata.h5ad")
```
...
#| tbl-cap: First several rows of the `merged_adata.var` to show the newly created `highly_variable` column.
#| echo: false
#| results: asis
...
::: {.callout-note}
# PCA
For a more **detailed explanation on PCA**,  there is a [self-learning lesson](06_theory_of_PCA.qmd) outlining the method. We also strongly encourage you to explore the video [StatQuest's video](https://www.youtube.com/watch?v=_UVHneBUBW0) as an alternative, step-by-step explanation of PCA.
:::
...
# Old version of the function have different arguments
# If participants run into problems, try running
# sc.pp.pca(
#     adata_phase,
#     n_comps=50,
...
Whenever we run a new function, it is a good idea to observe how our scanpy object changes. Since this is a score being calculated for each cell, we can take a look at the metadata to see which new columns have been generated.
...
```{r}
#| label: fig-pca_cell_cycle_split
#| fig-cap: Cells plotted on PC1 vs PC2 and colored by cell cycle phase.
#| fig-width: 9
...
The scanpy workflow does not have the exact `SCTransform` algorithm built into its workflow. However, there is an analagous [Analytic Pearson Residuals](https://www.sc-best-practices.org/preprocessing_visualization/normalization.html#analytic-pearson-residuals) normalization that follows the same logic. 
