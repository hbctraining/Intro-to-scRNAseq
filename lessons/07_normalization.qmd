---
title: "Normalization and exploring data for unwanted variation"
description: |
...
Before we do any normalization, we want to ensure that the raw counts matrix is properly stores as a `layer` without our object. By default, when we loaded in our dataset, the counts were stored to the `.X` slot of our object. So now we are going to store it in the `layers` slot under the name `counts` for easy reference in the future.
...
So now we have three layers, each with different counts:
...
Highly variable gene selection is extremely important since many downstream steps are computed only on these genes.  We can additionally visualize the dispersion of all genes, which shows a gene's average expression across all cells on the x-axis and variance on the y-axis. Ideally we want to use genes that have high variance since this can indicate a change in expression depending on populations of cells. Adding labels helps us understand which genes will be driving shape of our data.

The variance is computed from the raw counts matrix.
...
TODO: add information about each column
...
# Label the top N most variable genes on the plot
N = 15
...
Now, we can perform the PCA analysis. These highly variable genes are part fo the input necessary for this next step.
...
::: {.callout-note}
# PCA
For a more **detailed explanation on PCA**,  there is a [lesson in self-learning](06_theory_of_PCA.qmd) outlining the method. We also strongly encourage you to explore the video [StatQuest's video](https://www.youtube.com/watch?v=_UVHneBUBW0) for a more thorough understanding.
:::
...
TODO: discuss the splitting by sample
...
Within the scanpy workflow, there is not analogous normalization method that follows SCTransform. 
...
We will be following an alternative method to deal with integration and batch correction in the next lesson. This technique, called SCVI requires different inputs.
