...
description: |
  This lesson introduces participants to the steps of setting up a single-cell RNA‑seq project. Participants will learn when to consider using scRNA-seq as well as the challenges that scRNA-seq provides.
author:
  - Mary Piper
  - Meeta Mistry
  - Radhika Khetani
...
- Explain why single-cell RNA-seq is used instead of bulk RNA-seq for studying cellular heterogeneity and gene expression at single-cell resolution.  
- Identify key biological and technical sources of variability in scRNA-seq data and how they can obscure true cell types or states.  
- Assess when scRNA-seq is appropriate for a research question compared to alternatives such as bulk RNA-seq.
...
::: {#fig-sc-cell_diversity .figure}
![](../img/scRNA-seq_cell_diversity.png){width=630}

Depiction of the microenvironment found in a variety of different tissues.<br> 
_Image credit:_ courtesy of Dr. Ayshwarya Subramanian.
:::
...
Contrasting how bulk sequencing averages expression against the cellular resolution of single-cell sequencing.<br>
_Image source:_ [Trapnell et al. (2015)](https://dx.doi.org/10.1101/gr.190595.115)
:::
...
Expression data from scRNA-seq experiments represent tens or hundreds of thousands of reads for thousands of cells. The data output is much larger, requiring more memory to analyze, larger storage requirements, and more time to run the analyses.
...
Uninteresting sources of biological variation (i.e date of sequencing, scientist conducting experiment, etc.) can cause cells that are transcriptionally similar to one another to appear dissimilar. These factors can even obscure the cell type identities. Uninteresting sources of biological variation (unless part of the experiment's study) include:
...
::: {#fig-sc-variability .figure}
![](../img/sc_biol_variability.png){width=500}

Different sources of variation that can be found in single-cell sequencing experiments.<br>
_Image source:_ [Wagner et al. (2016)](https://dx.doi.org/10.1038%2Fnbt.3711)
:::

## Technical variability across cells/samples

Technical sources of variation can result in gene expression between cells being more similar/different based on technical sources instead of biological cell types/states, which can obscure the cell type identities. 

### Sources of technical variation

Technical sources of variation include:
...
::: {#fig-sc-batch_pca .figure}
![](../img/batch_effect_pca.png){width=600}

Example PCA, showcasing strong technical batch effect.<br>
_Image source:_ [Hicks et al. (2015)](https://www.biorxiv.org/content/early/2015/08/25/025528)
:::
...
Design the experiment in a way to **avoid batches**, if possible. If you are unable to avoid batches:

**Do NOT confound** your experiment by batch:

::: {#fig-sc-confounded .figure}
![](../img/confounded_batch.png){width=300}

Confounded batch effect.<br>
_Image source:_ [Hicks et al. (2015)](https://www.biorxiv.org/content/early/2015/08/25/025528)
:::
  
**DO** split replicates of the different sample groups across batches. The more replicates the better (definitely more than 2), if doing DE across conditions or making conclusions at the population level. If using inDrops, which prepares a single library at a time, alternate the sample groups (e.g. don't prepare all control libraries first and then prepare all treatment libraries).

::: {#fig-sc-batch_effect .figure}
![](../img/batch_effect.png){width=300}

Replicates that are mixed across batches to overcome batch effects.<br>
_Image source:_ [Hicks et al. (2015)](https://www.biorxiv.org/content/early/2015/08/25/025528)
:::
...
- Avoid technical sources of variability, if possible:
	...
	- Prepare libraries at the same time or alternate sample groups to avoid batch confounding
...
_We will not be covering snRNA-seq in this workshop!_ Below is a brief overview of snRNA-seq.
...
[Next Lesson >>](02_generation_of_count_matrix.qmd)
