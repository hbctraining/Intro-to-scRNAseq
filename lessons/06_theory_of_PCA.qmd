...
title: "Theory of PCA"
description: |
  This lesson introduces participants to the theory and mechanics of Principal Components Analysis (PCA). Through a step‑by‑step, hands‑on walkthrough, participants learn how to derive the covariance matrix, obtain eigenvalues and eigenvectors, compute principal component scores and compare their manual PCA results with the output of `prcomp()`. The lesson emphasizes visualization and understanding how PCA operates on high‑dimensional single‑cell RNA‑seq data.
author:
  - Will Gammerdinger
  - Noor Sohail
  - Shannan Ho Sui
...
# Use a clean default style for plots
...
```{python}
#| label: plot_raw_data_py
# Create a plot to view the raw expression data
fig, ax = plt.subplots()

ax.scatter(expression_df["Gene_A"], expression_df["Gene_B"],
           color="cornflowerblue")

for _, row in expression_df.iterrows():
    ax.text(row["Gene_A"], row["Gene_B"], row["cells"],
            ha="left", va="bottom")
...
# Create a dictionary to hold the center of the data
center_of_data_py = {
    "Gene_A": Gene_A_mean,
    "Gene_B": Gene_B_mean
}
...
ax.scatter([Gene_A_mean], [Gene_B_mean], color="red", s=50)
ax.text(Gene_A_mean, Gene_B_mean, "New Center",
...
```{python}
#| label: recentered_data_py
# Shift the data points so that the data is centered on the origin
recentered_expression_df = expression_df.copy()
recentered_expression_df["Gene_A"] = recentered_expression_df["Gene_A"] - Gene_A_mean
recentered_expression_df["Gene_B"] = recentered_expression_df["Gene_B"] - Gene_B_mean

# Print out the re-centered data
recentered_expression_df
```
...
```{python}
#| label: plot_recentered_data_py
# Plot the raw data after it has been shifted to have the center of the data align with the origin
fig, ax = plt.subplots()

ax.scatter(recentered_expression_df["Gene_A"],
           recentered_expression_df["Gene_B"],
           color="cornflowerblue")

for _, row in recentered_expression_df.iterrows():
    ax.text(row["Gene_A"], row["Gene_B"], row["cells"],
            ha="left", va="bottom")

# Mark the new center at the origin
ax.scatter([0], [0], color="red", s=50)
ax.text(0, 0, "New Center", color="red", ha="left", va="bottom")
...
```{r}
#| label: create_covariance_matrix
...
```{python}
#| label: create_covariance_matrix_py
# Move the cell IDs to the index and convert the DataFrame to a NumPy array
recentered_expression_matrix_py = (
    recentered_expression_df.set_index("cells")[["Gene_A", "Gene_B"]].to_numpy()
)

# Create a covariance matrix (rowvar=False because columns are variables)
cov_matrix_py = np.cov(recentered_expression_matrix_py, rowvar=False)

# Wrap in a DataFrame for nicer display
cov_matrix_py_df = pd.DataFrame(
    cov_matrix_py,
    index=["Gene_A", "Gene_B"],
...
```{r}
#| label: handcheck_covariance_matrix
...
```{python}
#| label: handcheck_covariance_matrix_py
# Estimate the covariance of Gene A and Gene B by hand
cov_hand = (1/(4-1)) * (
    (0 - 25) * (4 - 27.25) +
    (12 - 25) * (30 - 27.25) +
...
# Or you can use NumPy's covariance function directly
cov_np = np.cov(
    recentered_expression_matrix_py[:, 0],
    recentered_expression_matrix_py[:, 1],
    bias=False
)[0, 1]
...
```{python}
#| label: eigen_calculation_py
# Find the eigenvalues and eigenvectors of the covariance matrix
eig_values_py, eig_vectors_py = np.linalg.eig(cov_matrix_py)

# For nicer display, wrap in a dict-like structure
eig_py = {
    "values": eig_values_py,
    "vectors": eig_vectors_py
}
eig_py
```
...
```{python}
#| label: check_eigen_py
# Check the eigenvalues and eigenvectors
check_vec = (cov_matrix_py - eig_values_py[0] * np.eye(2)).dot(eig_vectors_py[:, 0])
check_vec
```
...
```{python}
#| label: pc_scores_py
# Transform the data into PC space by multiplying the re-centered expression matrix by the eigenvectors
pc_scores_py = recentered_expression_matrix_py.dot(eig_vectors_py)

# Name the columns in pc_scores object
pc_scores_df_py = pd.DataFrame(
    pc_scores_py,
    index=recentered_expression_df["cells"],
    columns=["PC_1", "PC_2"]
)
...
```{python}
#| label: percent_explained_py
# Calculate the percent of variance explained by each PC using the eigenvalues
pct_var_explained_py = (eig_values_py / eig_values_py.sum()) * 100

# Wrap in a Series with PC names
pct_var_explained_series_py = pd.Series(
    pct_var_explained_py,
    index=["PC_1", "PC_2"]
)
pct_var_explained_series_py
```
...
```{python}
#| label: plotting_PCA_hand_py
# Create a DataFrame to hold the PC scores
pc_scores_tibble_py = pc_scores_df_py.reset_index().rename(columns={"index": "cells"})

# Plot the PC scores we found
fig, ax = plt.subplots()

ax.scatter(pc_scores_tibble_py["PC_1"],
           pc_scores_tibble_py["PC_2"],
           color="cornflowerblue")

for _, row in pc_scores_tibble_py.iterrows():
    ax.text(row["PC_1"], row["PC_2"], row["cells"],
            ha="left", va="bottom")

ax.set_xlim(-50, 50)
ax.set_ylim(-12, 12)

xlab = f"PC 1 (Variance Explained {pct_var_explained_series_py['PC_1']:.2f}%)"
ylab = f"PC 2 (Variance Explained {pct_var_explained_series_py['PC_2']:.2f}%)"
...
```{python}
#| label: prcomp_check_py
# Run PCA using NumPy/linear algebra on the expression dataframe
# (after moving the Cell IDs to be the index)
expression_matrix_py = expression_df.set_index("cells")[["Gene_A", "Gene_B"]].to_numpy()

# Center the data as prcomp() does
center_py = expression_matrix_py.mean(axis=0)
expression_centered_py = expression_matrix_py - center_py

# Compute covariance and eigen-decomposition
cov_matrix_prcomp_py = np.cov(expression_centered_py, rowvar=False)
eig_vals_prcomp_py, eig_vecs_prcomp_py = np.linalg.eig(cov_matrix_prcomp_py)

prcomp_PCA_py = {
    "center": center_py,
    "sdev": np.sqrt(eig_vals_prcomp_py),
    "rotation": eig_vecs_prcomp_py,
    "x": expression_centered_py.dot(eig_vecs_prcomp_py)
}
prcomp_PCA_py
```
...
```{python}
#| label: center_prcomp_py
# Print the center of the data found by the Python "prcomp" analog
prcomp_PCA_py["center"]
```
...
```{python}
#| label: eigenvalues_prcomp_py
# Print the eigenvalues found by the Python "prcomp" analog
prcomp_eigenvalues_py = prcomp_PCA_py["sdev"] ** 2
prcomp_eigenvalues_py
```
...
```{python}
#| label: eigenvector_prcomp_py
# Print the eigenvectors found by the Python "prcomp" analog
prcomp_PCA_py["rotation"]
```
...
```{python}
#| label: pc_scores_prcomp_py
# Print the PC scores found by the Python "prcomp" analog
prcomp_PCA_py["x"]
```
...
[Next Lesson >>](07_SCT_normalization.qmd)
