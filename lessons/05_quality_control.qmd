...
The first step is to identify which genes are mitochondrial. We do this my identify any genes (from the `.var`) that starts with the character `MT-` and assign `True`/`False` values to genes that fit that criteria.
...
metadata_adata.var.head()
```

```{python}
#| label: tbl-show_var_head_py
#| tbl-cap: "Inspecting our gene metadata table to see our mitochondrial information."
#| echo: false
#| results: asis
mito_genes = merged_adata.var_names.str.startswith('MT-')
merged_adata.var['mito'] = mito_genes

print(merged_adata.var.head().to_markdown())
```
...
#| fig-cap: Density plot showing the distribution of mitochondrial gene expression detected per cell for each sample.
# Histogram of mitochondrial ration
ax = sns.histplot(merged_adata.obs, 
