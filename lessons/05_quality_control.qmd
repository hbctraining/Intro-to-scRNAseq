...
When data is loaded into Seurat and the initial object is created, there is some basic metadata asssembled for each of the cells in the count matrix. To take a close look at this metadata, let's view the data frame stored in the `metadata` slot of our `merged` object:
...
To calculate some of our QC metrics, we can use the `calculate_qc_metrics()` function to automatically generate scores for each cell.
...
```{python}
#| label: tbl-py_show_metadata_head
#| tbl-cap: "Inspecting our metadata table to get a sense for the type of information stored within it."
#| echo: false
#| results: asis
sc.pp.calculate_qc_metrics(merged_adata,
...
#| tbl-cap: "Inspecting the first ten rows of our metadata table to ensure that they look formatted correctly."
#| echo: false
metadata %>% 
  head(n = 5) %>% 
  remove_rownames() %>%
...
# Visualize the number UMIs/transcripts per cell
fig, ax = plt.subplots()
ax = sns.histplot(merged_adata.obs, x="total_counts", hue="sample",
...
#| eval: false
# Visualize the distribution of genes detected per cell via histogram
ax = sns.histplot(merged_adata.obs, x="n_genes", hue="sample",
                  kde=True, # adds line showing general trend
                  log_scale=True, # apply log scale to counts
                  alpha=0.2)
